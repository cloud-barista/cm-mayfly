...
### 종료 코드
`rest` 명령은 응답 상태 코드를 프로세스 종료 코드로 반영합니다(docker-compose healthcheck 등이 이에 의존합니다).

- 2xx 응답(201 Created·204 No Content 포함)이면 종료 코드 `0`(성공)
- 그 외 상태 코드는 상태 코드의 첫 자리(`3`·`4`·`5`)를 종료 코드로 반환
- 연결 실패 등 전송(transport) 오류로 응답을 받지 못하면 종료 코드 `1`

### 요청 타임아웃
각 요청에는 기본 30초 타임아웃이 적용되어, 응답이 오지 않는 호출이 무한정 매달리지 않고 실패합니다. 필요하면 `MAYFLY_HTTP_TIMEOUT` 환경 변수(정수 초 단위)로 값을 조정할 수 있습니다. 또한 GET/HEAD처럼 멱등(idempotent)한 요청은 전송 오류나 일시적 `502`/`503`/`504` 응답에 대해 최대 2회 재시도합니다(그 외 메서드와 4xx는 재시도하지 않습니다).

```bash
# 요청 타임아웃을 60초로 지정
$ MAYFLY_HTTP_TIMEOUT=60 ./mayfly rest get http://localhost:1323/tumblebug/readyz
```
