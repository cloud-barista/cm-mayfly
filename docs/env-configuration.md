...
- [4. VAULT_TOKEN — the one runtime-generated value](#4-vault_token--the-one-runtime-generated-value)
...
`.env.example` already carries sensible defaults for every **non-secret** value (log levels, DB user/name, API usernames, `VAULT_ADDR`, the unseal poll interval, …), so in practice you only need to fill the blank **secret** entries. If `conf/docker/.env` is missing, or a required value is blank, `mayfly`'s `infra` commands stop with a clear error **before** running docker compose — so you never start the stack with unset variables.

---

## 2. Required vs optional

**Every variable declared in `.env.example` is required (must be non-empty) — except two groups that are allowed to be blank:**
...
| `VAULT_TOKEN` | Intentionally blank on a fresh install — it is generated and written by the OpenBao init flow during `mayfly infra run` (see [§4](#4-vault_token--the-one-runtime-generated-value)). |

Everything else must be set. This is stronger than "just fill the passwords": it means a value you **accidentally blank or delete** — even a non-secret one that ships with a default — is caught before the stack starts. The reason is that a container which reads a blank value has **no safe built-in default of its own**, and because values are passed between containers, a downstream container cannot be assumed to know the right default for an upstream one. Requiring everything (except the two explicit exceptions) removes that whole class of silent, hard-to-diagnose failures.

The pre-run check runs automatically as part of every `mayfly infra` subcommand. Example when something is blank or missing:

```
Error: required values are missing or blank in conf/docker/.env:
  - TUMBLEBUG_DB_USER

Every variable in conf/docker/.env.example must be set, because a container that reads a blank
value has no safe built-in default (e.g. cb-spider 0.12.17+ exits on blank REST auth; a postgres
healthcheck `pg_isready -U ${*_DB_USER}` fails on a blank user and deadlocks every dependent service).
Only the cm-cicada SMTP_* settings and VAULT_TOKEN (auto-generated on first run) may be left blank.
...
| `VAULT_TOKEN` | ⚪ optional | (blank) | cb-tumblebug, mc-terrarium — see [§4](#4-vault_token--the-one-runtime-generated-value) |
| `OPENBAO_UNSEAL_POLL_INTERVAL` | ✅ | `30` | openbao-unseal |
